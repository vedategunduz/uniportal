...
@section('title', 'Kamular')

@section('content')
    <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4" id="kamular">
        @foreach ($kamular as $kamu)
            <x-kamu logoUrl="{{ $kamu->logoUrl }}" websiteUrl="{{ $kamu->websiteUrl }}" xUrl="{{ $kamu->xUrl }}"
                instagramUrl="{{ $kamu->instagramUrl }}" linkedinUrl="{{ $kamu->linkedinUrl }}" text="{{ $kamu->baslik }}"
